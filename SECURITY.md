...
Potter keeps every provider API key in the Python process. The SwiftUI client
only receives a separate local-server access token and sends a public model ID.
...
- Do not put any provider API key in the iOS source or app settings.
...
- `POTTER_DATA_DIR` stores continuation identifiers and limited provider chat
  history, not API keys. Protect it like other application state.
